    ...
    def average_pf(self):
        ...
        for frame in nas_dict.keys():
            ...
            for pt_type in nas_dict[frame].keys():
                ...
                pf.loc[(pf.frame == frame) & (pf.type == pt_type) & (pf.landmark_index.isin(nas_dict[frame][pt_type])), 'x'] = [new_x[i-1] for i in lndmrks_pos]
                pf.loc[(pf.frame == frame) & (pf.type == pt_type) & (pf.landmark_index.isin(nas_dict[frame][pt_type])), 'y'] = [new_y[i-1] for i in lndmrks_pos]
                pf.loc[(pf.frame == frame) & (pf.type == pt_type) & (pf.landmark_index.isin(nas_dict[frame][pt_type])), 'z'] = [new_z[i-1] for i in lndmrks_pos]
